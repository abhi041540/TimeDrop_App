...
function ForgotPassword() {
    ...
    function changepassword(event) {
        ...
        if (!(password.length >= 8 && password.length <= 20)) {
            ...
            axios.post(`${surl}/updatedpassword`, { password: password, email: email, otp: otp }).then((resp) => {
                ...
                if (resp.data === "noemail") {
                    ...
                    event.target.elements["email"].value = "";
                    event.target.elements["password"].value = "";
                    $(".pcotp").css({ "visibility": "hidden" });
                }
                else if (resp.data === "otp_sent") {
                    $(".pcotp form").on("submit", (event1) => {
                        ...
                        if (uotp === otp) {
                            ...
                            event.target.elements["email"].value = "";
                            event.target.elements["password"].value = "";
                            axios.post(`${surl}/otpchangesave`, { password: password, email: email }).then((resp1) => {
                                if (resp1.data != "successfull") {
                                    alert("Somthing went wrong change password unsaved try again later!");
                                }
                            });
                        }
                        else {
                            alert("wrong otp process failed");
                            $("#pcotpinput").attr("value", "");
                            $(".pcotp").css({ "visibility": "hidden" });
                            event.target.elements["email"].value = "";
                            event.target.elements["password"].value = "";
                        }
                    });
                    $(".pcotp").css({ "visibility": "visible" });
    ...
    return (
        <section className="loginform forgotpassword">
                ...
                <div class="row">
                    ...
                    <div style={{ textAlign: "center" }} className="col">
                        <div style={{ textAlign: "center", minWidth: "300px", maxWidth: "600px", margin: "0 auto" }}>
                            <div>
                                <h1 style={{ fontFamily: "serif", fontWeight: 600, fontSize: "160%", zIndex: 200, position: "relative" }}>Change Password</h1>
                            </div>
                            <form method="post" style={{ display: "block", paddingBottom: "40px", zIndex: 200, position: "relative" }} onSubmit={changepassword}>
                                <div class="mb-3">
                                    <label class="form-label">Registerd Email</label>
                                    <input onFocus={inpborder} type="email" name="email" class="form-control" placeholder="Enter Email Here" required />
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">New Password</label>
                                    <input onFocus={inpborder} type="password" name="password" class="form-control" placeholder="Enter New Password Here" required />
                                ...
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", marginTop: "40px" }}>
                                    ...
                                    <button className="signup_login" id="fback" style={{ marginLeft: "20px" }}>Back</button>
                                </div>
                            </form>
                        </div>
                    </div>
